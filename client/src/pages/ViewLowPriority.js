...
import { LowPriorityTask } from '../actions/actions';
import TaskCard from '../components/cards/TaskCard';
import { motion as m } from 'framer-motion';

function ViewLowTasks() {
    ...
    useEffect(() => {
        if(tasks.length <=0) {
            const fetchData = async () => {
                try{
                    let data = await LowPriorityTask();
                    setTasks(data);
                    console.log(data)
    ...
    return (
        <div className='view-tasks'>
            <Container>
                ...
                <p> You Currently Have {tasks.length} Low Priority Tasks </p>
